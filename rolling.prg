...
genr y=100*dlog(adj_close) 'generate return series, 1/4/2012--4/20/2021
scalar alpha=0.05 ' set VaR level at 5%
...
for !i=1 to N  'use "for" loop
	...
	genr uhat2=uhat*uhat ' generate sq error
...
smpl 02/01/2018 4/20/2021  'out-of-sample period
group VaR VaR_GARCH VaR_TGARCH  'group VaR forecasts from GARCH and TAGARCH and name the group "VaR"
group Qhat Qhat_GARCH Qhat_TGARCH 'group the check function Qhat (now 0) from VaR_GARCH and VaR_TAGARCH and name the group "Qhat"
group alphahat alphahat_GARCH alphahat_TGARCH  'group alphahat (now 0) from VaR_GARCH and VaR_TAGARCH and name the group "alphahat"
for !i=1 to 2 'assign values to Qhat and  alphahat
	alphahat(!i)=y<VaR(!i)
...
delete yy uhat h eq1 eq2 open close low high volume 'delete objects not needed

save \\tsclient\D\sp500.wf1 'save results into a workfile; for local user, ``save  D:\sp500.wf1"
exit 'exit Eviews
